...
| Quality and validation | Dependency vulnerability triage baseline                          | `partial`         | Python requirements audit cleanly and repo validation passes, but npm still carries blocked `next` and GraphQL codegen alerts pending upstream.   |
